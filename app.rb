...
class App < Sinatra::Base
  ...
  post '/surveys' do
    ...
    survey = Survey.new(career_id: resulting_career,
  ...
  get '/surveys/search' do
    ...
    @surveys = Survey.where(Sequel.lit(
                              'career_id = ? AND created_at > ? AND created_at < ?', params[:careerId].to_i, DateTime.parse(params[:firstDate]), DateTime.parse(params[:lastDate]).next_day
                            ))
